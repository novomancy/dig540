...
class  Book{
  ...
  public function update(){
    ...
    if($db){
      $db->beginTransaction();
      $st = 'UPDATE Book SET ';
			$st.= 'bookid = "'.$this->bookid.'", ';
			$st.= 'book_title = "'.$this->book_title.'", ';
			$st.= 'book_num = "'.$this->book_num.'", ';
			$st.= 'story_serial = "'.$this->story_serial.'", ';
      $st.= 'story_serial_number = "'.$this->story_serial_number.'", ';
			$q = $db->prepare($st);
			try{
				$q->execute();
			} catch(PDOException $e) {
