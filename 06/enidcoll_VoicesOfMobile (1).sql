...
--
-- Host: localhost:3306
-- Generation Time: Oct 27, 2019 at 01:26 AM
...
(1, 'Peanut Man', '1922-2005', 'Lamar Wilson, also known as Mobile Alabama\'s Peanut Man was known for selling hot paper bags of Peanuts from the A and M Peanut Store on Dauphin Street on the sidewalks and squares of  Mobile, AL.'),
...
CREATE TABLE `recording` (
  ...
  `recording_permission_id` int(11) NOT NULL,
...
INSERT INTO `recording` (`id`, `title`, `recording_permission_id`, `producer`, `source`, `year`, `subject`, `extra`) VALUES
(1, 'Songs from the Peanut Man and Other Sidewalk Vendors', 1, 'John A. Lomax Southern States Collection', 'loc.gov/folklife/guides/streetcries.html', 1937, 'Field Recording', 'Original Collection contains 647 items on discs of varying sizes (8-12 inch discs.)'),
(2, 'The Story of Rain in Mobile, AL', 2, 'Mary A. Poole', 'alabamapioneers.com/story-of-rain-in-mobile-alabama/', 1936, 'folk story', 'Length not provided.'),
(3, 'We Remember interview with Holocaust Survivor Mrs. Agnes Tennenbaum, Mobile AL resident', 3, 'Victoria Hirsch and Cory Garfunkel', '\"We Remember\" produced by Gary Scovill with Kimberly and Stanley Zimmerman', 2012, 'Holocaust Survivor', 'Film Score Brian Keane. Interviewees included Agnes Tennenbaum, Violet Hirsch, James Philpot, Sibylle Von Campe Heidrich, and Roland Gulp Frey.'),
(4, 'Art IS Mobile Part 4: Monologue with Eugene Walter', 4, 'Melinda Klotz Mercer', 'https://youtu.be/IY01PE_F-Jg', 2016, 'Eugene Walter Interview', 'Local historian, and Mobile\'s own screenwriter, poet, short-story author, actor, puppeteer, gourmet chef, cryptographer, translator, editor, costume designer and well-known raconteur reminisces on his childhood and Mobile histories relating to it. Copyright 1999-2016.'),
...
(6, 'Dot Moore Mardi Gras Mobile, AL 1976', 6, 'WALA Channel 10 Local NBC Affiliate ', 'https://youtu.be/dwhPKL_Cgvo', 1976, 'Mobile Mardi Gras', 'Local news personality Dot Moore interviews George Criminale about building Mardi Gras Floats in Mobile AL 1976. Uploaded by Roland Fields.');
...
INSERT INTO `recording_permission` (`id`, `permission_name`) VALUES
(2, 'Alabama Pioneers'),
(5, 'Copyright Eugene Walter'),
(3, 'Copyright Scoville Productions'),
(1, 'loc.gov'),
(4, 'Public Domain YouTube'),
(6, 'YouTube Provided link');
...
INSERT INTO `recording_track` (`id`, `recording_id`, `genre_id`, `title`, `extra`) VALUES
(1, 1, 1, 'Sidewalk Chants from Mobile\'s Peanut Man', 'Field Recordings taken from the 1930\'s of a young Peanut Man hawking bags of peanuts in downtown Mobile, AL'),
(2, 1, 1, 'Mobile Mardi Gras emblem walkers announce the Order of Myths parade, 1947', 'Clip says \"Here comes the double O M\'s!\" of unknown announcer'),
(3, 2, 2, 'Story of Rain', 'On May 1st the German Relief gave a picnic and it was a standing joke, that Mobile would have rain when the German Relief had their picnic. However, they held to their date of May 1st even though it usually meant not only one but sometimes two postponement'),
(4, 3, 3, 'Interview with Holocaust Survivor Mrs. Agnes Tennennbaum', 'Approximately one hour'),
(5, 4, 4, 'Art is Mobile Monologue with Eugene Walter', 'Includes cover art by the author on original recording.'),
(6, 5, 5, 'Rare Bird poem', 'approximately 3 minutes'),
(7, 5, 2, 'Byzantine Riddle', 'Story read by the author Eugene Walter'),
(8, 6, 6, 'Mardi Gras News Cast from 1976 featuring Dot Moore\r\n\r\n\r\n', 'Dot Moore conducts her annual Mardi Gras newscast from downtown Mobile, AL');
...
ALTER TABLE `recording`
  ADD PRIMARY KEY (`id`),
  ADD KEY `title` (`title`),
  ADD KEY `recording_permission_id` (`recording_permission_id`);
...
ALTER TABLE `recording_genre`
  ADD KEY `recording_genre_ibfk_1` (`genre_id`),
  ADD KEY `recording_genre_ibfk_2` (`recording_id`);
...
--
-- AUTO_INCREMENT for table `recording_permission`
--
ALTER TABLE `recording_permission`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
...
--
ALTER TABLE `recording`
  ADD CONSTRAINT `recording_ibfk_1` FOREIGN KEY (`recording_permission_id`) REFERENCES `recording_permission` (`id`);
...
--
ALTER TABLE `recording_genre`
  ADD CONSTRAINT `recording_genre_ibfk_1` FOREIGN KEY (`genre_id`) REFERENCES `recording` (`id`),
  ADD CONSTRAINT `recording_genre_ibfk_2` FOREIGN KEY (`recording_id`) REFERENCES `genre` (`id`);

--
-- Constraints for table `recording_track`
--
ALTER TABLE `recording_track`
  ADD CONSTRAINT `recording_track_ibfk_1` FOREIGN KEY (`genre_id`) REFERENCES `genre` (`id`),
  ADD CONSTRAINT `recording_track_ibfk_2` FOREIGN KEY (`recording_id`) REFERENCES `recording` (`id`);
COMMIT;
