    ...
    class Album{
        ...
        public function getGenres(){ 
            for($j=0; $j<count($this->genres); $j++){
                if($j%2==0) print_r("<span style='color:red'>Genre #".($j+1)." is ".$this->genres[$j]."</span><br>");
                else print_r("<span style='color:blue'>Genre #".($j+1)." is ".$this->genres[$j]."</span><br>");
            }
        }
        ...
        public function getSubgenres(){ 
            for($j=0; $j<count($this->subgenres); $j++){
                if($j%2==0) print_r("<span style='color:red'>Subgenre #".($j+1)." is ".$this->subgenres[$j]."</span><br>");
                else print_r("<span style='color:blue'>Subgenre #".($j+1)." is ".$this->subgenres[$j]."</span><br>");
            }
        }

        public function setData($artistName, $titleName, $rankNumber, $releaseYear, $genres, $subs){
            $this->setArtist($artistName);
            $this->setTitle($titleName);
            $this->setRank($rankNumber);
            $this->setYear($releaseYear);
            $this->setGenres($genres);
            $this->setSubgenres($subs);
        }
